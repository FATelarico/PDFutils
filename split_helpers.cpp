...
/*
    This file assumes merge_helpers.cpp has already been included before it.
...
static QVector<PageRange> makeOnePageSplitRanges(
    int pageCount,
    QString& error)
{
...
static QVector<PageRange> makeTwoPageSplitRangesAfterEvenPages(
    int pageCount,
    QString& error)
{
    QVector<PageRange> ranges;
...
static QVector<PageRange> makeTwoPageSplitRangesAfterOddPages(
    int pageCount,
    QString& error)
{
    QVector<PageRange> ranges;
    ...
    /*
        ...
        - pageCount 3:  1, 2-3
        - pageCount 4:  1, 2-3, 4
        - pageCount 5:  1, 2-3, 4-5

        This follows the requested split boundary rule. It cannot make every
        output exactly two pages because the first boundary is after page 1.
    */
...
static bool parsePositiveIntegerToken(
    const QString& token,
    int& value,
    QString& error)
{
...
static bool parseCommaSeparatedSplitPoints(
    const QString& input,
...
static bool splitPointsToRanges(
    ...
    QString& error)
{
    ...
    for (int splitPoint : splitPoints) {
        ...
        if (splitPoint < firstPage) {
            error = QString("Invalid repeated or descending split page: %1.")
            .arg(splitPoint);
            return false;
        }
...
static bool determineSplitRangesFromManualSplitPoints(
    const QString& input,
    ...
    QString& error)
{
    ...
    if (documentSpecs.size() == 1) {
        ...
        if (!parseCommaSeparatedSplitPoints(
                documentSpecs[0],
                commonSplitPoints,
                parseError)) {
            error = parseError;
    ...
    for (int docIndex = 0; docIndex < pageCounts.size(); ++docIndex) {
        ...
        if (!splitPointsToRanges(
                splitPointsByDocument[docIndex],
                pageCounts[docIndex],
                ranges,
                rangeError)) {
            error = QString("Document %1: %2")
...
static bool makeEveryNPagesSplitRanges(
    int pageCount,
    int pagesPerPart,
    QVector<PageRange>& ranges,
    QString& error)
...
static bool determineSplitRangesEveryNPages(
    int pagesPerPart,
    const QVector<int>& pageCounts,
    QVector<QVector<PageRange>>& splitRangesByDocument,
    QString& error)
...
static std::pair<int, int> qpdfObjectKey(const QPDFObjectHandle& object)
{
...
static bool bookmarkTargetPagesToSplitRanges(
    ...
    QString& error)
{
    ...
    for (int targetPage : bookmarkTargetPages) {
        /*
            A bookmark targeting page N means that page N should start a new
            output file. Therefore the split point is N - 1.

            A bookmark targeting page 1 cannot create a split point.
        */
        if (targetPage <= 1) {
            continue;
        }
...
static bool determineSplitRangesFromBookmarksForOnePdf(
    ...
    QString& error)
{
    ...
    try {
        ...
        if (!bookmarkTargetPagesToSplitRanges(
                levelIt->second,
                pageCount,
                ranges,
                rangeError)) {
...
static bool determineSplitRangesFromBookmarks(
    ...
    QString& error)
{
    ...
    for (int docIndex = 0; docIndex < pdfPaths.size(); ++docIndex) {
        ...
        if (!determineSplitRangesFromBookmarksForOnePdf(
                pdfPaths[docIndex],
                requestedLevel,
                ranges,
                localError)) {
...
static bool determineSplitRangesFromPreferences(
    bool check18IsChecked,
    int combo18Index,
    bool check20IsChecked,
    const QString& lineEdit20Text,
    bool check21IsChecked,
    int spin21Value,
    bool check22IsChecked,
    int spin22Value,
    const QStringList& pdfPaths,
    const QVector<int>& pageCounts,
    QVector<QVector<PageRange>>& splitRangesByDocument,
    QString& error)
{
    ...
    /*
        Priority:
        1. check22: split before bookmarks of the requested level
        2. check21: split every n pages
        3. check20: split at manually listed page numbers
        4. check18: fixed predefined split mode
    */
    ...
    if (!check18IsChecked) {
        error = "No split method is selected.";
        return false;
    }
    ...
    for (int docIndex = 0; docIndex < pageCounts.size(); ++docIndex) {
        ...
        default:
            error = QString("Unknown split mode: %1.").arg(combo18Index);
            return false;
        }
        ...
        if (ranges.isEmpty()) {
            error = QString("Document %1 produced no split ranges.")
            .arg(docIndex + 1);
...
static bool validateSplitRange(
...
static bool splitOnePdfWithLibQpdf(
    const QString& inputPdfPath,
...
static bool splitPdfsWithLibQpdf(
    const QStringList& pdfPaths,
...
static bool validateSplitRangesAgainstPageCounts(
    const QVector<QVector<PageRange>>& splitRangesByDocument,
    const QVector<int>& pageCounts,
    QString& error)
{
    ...
    for (int docIndex = 0; docIndex < splitRangesByDocument.size(); ++docIndex) {
        ...
        if (ranges.isEmpty()) {
            error = QString("Document %1 has no split ranges.")
            .arg(docIndex + 1);
            return false;
        }
