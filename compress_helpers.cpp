 ...
 *
 *    Required existing definitions:
 *    - static QByteArray toQpdfFileName(const QString& path)
 *    - static QString normalisedPdfPath(const QString& path)
 *    - static bool makeSplitOutputPath(...)
 *
 *    Compression model:
 *    - qpdf safe structure optimisation uses qpdf's lossless structural options.
 *    - qpdf image optimisation adds qpdf's --optimize-images. qpdf may use lossy
 *      JPEG compression for supported non-JPEG images when this reduces size.
 *    - Ghostscript compression uses the pdfwrite device through QProcess.
 *    - Ghostscript preset mode uses -dPDFSETTINGS and -dCompatibilityLevel.
 *    - Ghostscript custom downsampling mode builds the explicit -o/-dDownsample...
 *      command requested by the UI and ignores preset/compatibility controls.
 *    - Metadata removal uses qpdf's --remove-info/--remove-metadata for qpdf.
 *      For Ghostscript preset mode, the helper appends a small pdfmark file that
 *      blanks standard document-info fields where possible.
 */
...
static QString formatCompressionByteSize(qint64 bytes)
{
...
static QString cleanAbsoluteCompressionPath(const QString& path)
{
    const QFileInfo fileInfo(path.trimmed());
    return QDir::cleanPath(fileInfo.absoluteFilePath());
}

static bool ensureCompressionOutputDirectoryExists(
...
static bool makeCompressionOutputPath(
    const QString& inputPdfPath,
    const QString& chosenOutputPath,
    int documentNumber,
    int documentCount,
    QString& compressionOutputPath,
    QString& error)
{
    compressionOutputPath.clear();
    error.clear();
    ...
    if (documentCount == 1) {
        compressionOutputPath = normalisedPdfPath(chosenOutputPath);

        if (!ensureCompressionOutputDirectoryExists(compressionOutputPath, error)) {
            compressionOutputPath.clear();
            return false;
        }
...
static QString ghostscriptPdfSettingsFromComboIndex(int comboIndex)
{
    switch (comboIndex) {
        case 0:
            return "/screen";
...
static QString ghostscriptCompatibilityLevelFromComboIndex(int comboIndex)
{
    switch (comboIndex) {
        case 0:
            return "1.4";
...
static QStringList ghostscriptProgramCandidates()
{
    #ifdef Q_OS_WIN
    return QStringList()
    << "gswin64c.exe"
...
static bool findGhostscriptProgram(
...
static QStringList qpdfCompressionArguments(
    const QString& inputPdfPath,
    const QString& outputPdfPath,
    const PdfCompressionOptions& options)
{
    QStringList args;
...
static bool runQpdfCompressionJob(
    const QString& inputPdfPath,
    const QString& outputPdfPath,
    const PdfCompressionOptions& options,
    QString& error)
{
    ...
    QStringList stringArgs = qpdfCompressionArguments(
        inputPdfPath,
        outputPdfPath,
        options
    );
    ...
    for (const QString& arg : qAsConst(stringArgs)) {
...
static bool createGhostscriptMetadataBlankingPdfmark(
...
static QStringList ghostscriptPresetCompressionArguments(
    const QString& inputPdfPath,
    const QString& outputPdfPath,
    const PdfCompressionOptions& options,
...
static QString ghostscriptBoolValue(bool value)
{
    return value ? "true" : "false";
}

static QString ghostscriptDoubleValue(double value)
{
    return QLocale::c().toString(value, 'g', 15);
}

static QStringList ghostscriptCustomDownsamplingArguments(
    const QString& inputPdfPath,
    const QString& outputPdfPath,
    const PdfCompressionOptions& options)
{
...
static bool runGhostscriptCompressionProcess(
    const QString& inputPdfPath,
    const QString& outputPdfPath,
    const PdfCompressionOptions& options,
    QString& error)
...
static bool compressOnePdfWithQpdf(
    const QString& inputPdfPath,
    const QString& outputPdfPath,
    const PdfCompressionOptions& options,
    PdfCompressionResult& result,
    QString& error)
{
    PdfCompressionOptions qpdfOptions = options;
    qpdfOptions.backend = PdfCompressionBackend::Qpdf;

    return compressOnePdf(
        inputPdfPath,
        outputPdfPath,
        qpdfOptions,
        result,
...
static bool compressOnePdfWithGhostscript(
    const QString& inputPdfPath,
    const QString& outputPdfPath,
    const PdfCompressionOptions& options,
    PdfCompressionResult& result,
    QString& error)
{
    PdfCompressionOptions ghostscriptOptions = options;
    ghostscriptOptions.backend = PdfCompressionBackend::Ghostscript;
...
static bool compressPdfsWithQpdf(
    const QStringList& inputPdfPaths,
    const QString& chosenOutputPath,
    const PdfCompressionOptions& options,
    QVector<PdfCompressionResult>& results,
...
static bool compressPdfsWithGhostscript(
    const QStringList& inputPdfPaths,
    const QString& chosenOutputPath,
    const PdfCompressionOptions& options,
    QVector<PdfCompressionResult>& results,
...
static QString makePdfCompressionReport(
